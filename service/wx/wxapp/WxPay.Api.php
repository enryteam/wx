...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj,$key1)
	{
		...
		//签名
		$inputObj->SetSign($key1);
		$xml = $inputObj->ToXml();
		
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::curlPost($url, $xml);
		
		$result = self::xml($response);
		//print_r($result);
		if ($result['return_code'] == 'SUCCESS' && $result['result_code'] == 'SUCCESS') {
			...
			$result1 = WxPayResults::InitFromArray($prePayParams,true)->GetValues();
			...
			$data['package'] = $pack;//统一下单接口返回的 prepay_id 参数值，提交格式如：prepay_id=*
			$data['partnerid'] = $result['mch_id'];
			$data['prepayid'] = $result['prepay_id'];
			$data['timestamp'] = $time_stamp;//时间戳
			//print_r($tmp);
			$data['sign'] = self::sign($tmp,$key1);//签名,具体签名方案参见微信公众号支付帮助文档;
			$aaaa = json_encode($data);
			echo $aaaa;
		}else{
			echo '参数错误';
		}
	...
	//签名 $data要先排好顺序
	public function sign($data,$key1){
			$stringA = '';
			...
			$wx_key = $key1;//申请支付后有给予一个商户账号和密码，登陆后自己设置的key
	...
	{		
	/*	$xml = '<xml>
		<appid><![CDATA[423423]]></appid>
		<body><![CDATA[订单支付]]></body>
		<mch_id>1330188401</mch_id>
		<nonce_str><![CDATA[nn5rx7us2s93uyvi9g395uaydi7jcphg]]></nonce_str>
		<notify_url><![CDATA[https://wx.enry.cn/zbk_rest/wxapp/notify.php]]></notify_url>
		<out_trade_no>20190316150242</out_trade_no>
		<spbill_create_ip><![CDATA[121.225.46.96]]></spbill_create_ip>
		<total_fee>100</total_fee>
		<trade_type><![CDATA[APP]]></trade_type>
		<sign><![CDATA[4038E8DFE1A7FAEB4ACD7E39B752C8E3]]></sign>
		</xml>';
	*/
		$ch = curl_init();
		//设置超时
